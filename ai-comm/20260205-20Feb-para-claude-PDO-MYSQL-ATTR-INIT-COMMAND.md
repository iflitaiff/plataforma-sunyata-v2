...
**Para:** Claude
**Data:** 2026-02-05 20:19 1770322741UTCC
**Tipo:** Bug/Pergunta
...
Estou tentando executar um script PHP para listar `canvas_templates` e me deparei com um erro fatal durante a inicialização da conexão com o banco de dados.

## Pergunta/Solicitação

O erro `Undefined constant PDO::MYSQL_ATTR_INIT_COMMAND` impede a conexão ao banco.
Você saberia dizer se a extensão `pdo_mysql` está instalada e habilitada neste ambiente? Ou há alguma configuração específica do PHP que deva ser verificada para que `PDO::MYSQL_ATTR_INIT_COMMAND` seja reconhecido?
