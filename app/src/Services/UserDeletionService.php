...
class UserDeletionService {
     ...
     * @param int|null $adminId ID do admin executando a ação (null para auto-deleção)
     * @return array ['success' => bool, 'message' => string, 'deleted_records' => array]
     */
    public function deleteUser(int $userId, ?int $adminId = null): array {
        try {
            ...
            $this->db->insert('audit_logs', [
                'user_id' => $adminId, // null para auto-deleção
                'action' => $adminId ? 'user_deleted_by_admin' : 'user_self_deleted',
                'entity_type' => 'users',
                'entity_id' => $userId,
     ...
     *
     * @param int $userId ID do usuário a deletar
     * @param int|null $adminId ID do admin executando (null para auto-deleção)
    ...
    private function validateDeletion(int $userId, ?int $adminId): array {
        ...
        // 2. Auto-deleção: permitir se for o próprio usuário e não for admin
        if ($adminId === null) {
            // Auto-deleção: não permitir se for admin
            if ($user['access_level'] === 'admin') {
                return [
                    'success' => false,
                    'message' => 'Administradores não podem deletar suas próprias contas por segurança.',
                    'user' => null
                ];
            }
            // Auto-deleção permitida para usuários normais
            return [
                'success' => true,
                'message' => 'Auto-deleção autorizada',
                'user' => $user
            ];
        }

        // 3. Deleção por admin: validações adicionais
