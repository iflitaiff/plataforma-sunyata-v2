<?php
/**
 * Canvas Jurídico v3 - Templates Padronizados (Manus Test)
 * Versão com templates padronizados e melhorias de UX/DX
 * Usa templates com sufixo -manus-test do banco de dados
 */
...
// Verificar se tem acesso (vertical juridico, iatr OU usuário demo/admin)
...
if (!$show_menu) {
    // Carregar template específico (V3: usa sufixo -manus-test)
    $canvas = $db->fetchOne("
        SELECT id, slug, name, form_config, system_prompt, user_prompt_template, max_questions
        FROM canvas_templates
        WHERE slug = :slug AND slug ILIKE '%-manus-test' AND is_active = TRUE
    ", ['slug' => $selected_aj]);

    if (!$canvas) {
        $_SESSION['error'] = 'Template não encontrado: ' . sanitize_output($selected_aj);
        redirect(BASE_URL . '/areas/juridico/canvas-juridico-v3.php');
    }
...
// Se modo menu, buscar todos os templates (V3: apenas -manus-test)
$all_templates = [];
if ($show_menu) {
    $all_templates = $db->fetchAll("
        SELECT id, slug, name
        FROM canvas_templates
        WHERE slug ILIKE '%-manus-test' AND is_active = TRUE
        ORDER BY
            CASE
                WHEN slug ILIKE '%-livre-manus-test' THEN 99
                ELSE id
            END
    ");
}

$pageTitle = $show_menu ? 'Canvas Jurídico v3 - Templates Padronizados' : $canvas['name'];
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <?php if (!$show_menu): ?>
    <!-- SurveyJS CSS - APENAS core, tema vem do JSON via applyTheme() - v2.4.1 (latest stable) -->
    ...
    <style>
        ...
        /* Estilizar títulos dos campos - cor vem do tema JSON */
        .sd-question__title {
            font-weight: 600;
            font-size: 1.1rem;
            margin-bottom: 8px;
        ...
        /* Espaçamento entre perguntas - cores vêm do tema JSON */
        .sd-question {
        ...
        /* Transição suave - cores vêm do tema JSON */
        .sd-question {
        ...
        /* Título da pergunta - cor vem do tema JSON */
        .sd-question__title {
        ...
        /* Botão de submit - cores vêm do tema JSON */
        .sd-btn {
        ...
        /* Progress bar - cores vêm do tema JSON */
        .sd-progress {
            border-radius: 10px !important;
...
<body>
    <div class="container-custom">
        ...
        <div class="canvas-nav">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    ...
                    <?php if (!$show_menu): ?>
                    <a href="<?= BASE_URL ?>/areas/juridico/canvas-juridico-v3.php" class="btn btn-sm btn-outline-primary">
                        📋 Menu de Atividades
                    </a>
                    <?php endif; ?>
                </div>
        ...
        <div class="header">
            <?php if ($show_menu): ?>
            <h1><?= sanitize_output($pageTitle) ?> <span class="badge bg-success">V3</span></h1>
            <p>📋 Templates padronizados com melhorias de UX e exemplos contextualizados</p>
            <?php endif; ?>
    ...
    <script>
         ...
         * @param {string} responseText - Texto da resposta (Markdown ou HTML)
         * @returns {string} HTML sanitizado pronto para exibição
         */
        function renderResponse(responseText) {
            ...
            // Detectar formato: HTML começa com tag (ignorando espaços em branco)
            ...
            // Sanitizar para segurança (sempre!)
        ...
        // Tema vem do JSON (não forçar tema hardcoded)
        // Se JSON não tiver tema, SurveyJS usa padrão
        if (surveyJson.themeName) {
            console.log('✅ Tema do JSON:', surveyJson.themeName);
        }

        // DEBUG: Ver JSON real sendo usado
        console.log('📋 Survey JSON:', JSON.stringify(surveyJson, null, 2));
        console.log('📋 showTitle:', surveyJson.showTitle);
        console.log('📋 showDescription:', surveyJson.showDescription);

        // Renderizar no container
        survey.render(document.getElementById("surveyContainer"));

        // DEBUG: Ver HTML gerado
        setTimeout(() => {
            console.log('📋 HTML gerado:', document.getElementById('surveyContainer').innerHTML.substring(0, 500));
        }, 100);
        ...
        survey.onComplete.add(async function (sender) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Mostrar resultado - Híbrido: detecta Markdown ou HTML automaticamente
